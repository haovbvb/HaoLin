...
+ (UIImage *) gradientImageWithSize:(CGSize) size locations:(const CGFloat []) locations components:(const CGFloat []) components
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context
{
    ...
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
#pragma mark --sizeof(locations)/sizeof(locations[0]) 第四个参数渐变色--
    //sizeof(locations)/sizeof(locations[0]) 第四个参数
    CGGradientRef gradient = CGGradientCreateWithColorComponents(colorSpace,components,locations,1);
    CGColorSpaceRelease(colorSpace);
...
- (void) showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems
{
    ...
    if (!_observing) {
        _observing = YES;
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(orientationWillChange:)
