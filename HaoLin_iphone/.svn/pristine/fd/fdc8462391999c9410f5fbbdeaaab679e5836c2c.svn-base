...
#define HSPSegmentedControl_Height 35.0
#define HSPSegmentedControl_Width ([UIScreen mainScreen].bounds.size.width)
...
@implementation HSPSegmentedControl
...
{
    CGRect rect4View = CGRectMake(.0f, y, HSPSegmentedControl_Width, HSPSegmentedControl_Height);
    if (self = [super initWithFrame:rect4View]) {
        ...
        _scrollView.contentSize = CGSizeMake([titles count]*width4btn, HSPSegmentedControl_Height);
        _scrollView.showsHorizontalScrollIndicator = NO;
        
        for (int i = 0; i<[titles count]; i++) {
            
            UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
            btn.frame = CGRectMake(i*width4btn, .0f, width4btn, HSPSegmentedControl_Height);
            [btn setTitleColor:UIColorFromRGBValue(0x666666) forState:UIControlStateNormal];
            btn.titleLabel.font = [UIFont systemFontOfSize:14];
            [btn setTitleColor:HSPBarBgColor forState:UIControlStateSelected];
        ...
        //  lineView
        //
        CGFloat height4Line = HSPSegmentedControl_Height/3.0f;
        CGFloat originY = (HSPSegmentedControl_Height - height4Line)/2;
        ...
        //  bottom lineView
        //
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(5.0f, HSPSegmentedControl_Height-3, width4btn-10.0f, 3.0f)];
        _bottomLineView.backgroundColor = HSPBarBgColor;
...
- (void)segmentedControlChange:(UIButton *)btn
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(HSPSegmentedControlSelectAtIndex:)]) {
        [self.delegate HSPSegmentedControlSelectAtIndex:btn.tag - 1000];
    }
}
