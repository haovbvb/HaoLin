...
-(void)awakeFromNib
{
    ...
    _subviewarry =[[NSMutableArray alloc]initWithCapacity:0];
    _tablearry   =[[NSMutableArray alloc]initWithCapacity:0];
...
-(void)reloadHeader
{
    [self.JYtableview addHeaderWithTarget:self action:@selector(requsetdata)];
    [self.JYtableview headerBeginRefreshing];
    
//    [self.JYtableview footerEndRefreshing];
}
-(void)reloadtable
{
    [self.JYtableview reloadData];
...
-(void)inittijiaoView
{
    ...
    goodstableview =[[UITableView alloc]initWithFrame:CGRectMake(0, 102, KDeviceWidth, KDeviceHeight-130) style:UITableViewStylePlain];
    ...
    FooterView.frame=CGRectMake(0, KDeviceHeight-KDeviceHeight*30/KDeviceHeight, KDeviceWidth,30);
     ...
     *  刷新全部商品View
     */
    refreshview=[[[NSBundle mainBundle]loadNibNamed:@"MDLRefreshView" owner:self options:nil]lastObject];
    refreshview.frame=CGRectMake(0, 109, KDeviceWidth, KDeviceHeight);
    UIImage *refreshimage=[UIImage imageNamed:@"MDLrefresh@2x"];
    [refreshview.Refreshbtn setBackgroundImage:refreshimage forState:UIControlStateNormal];
    [refreshview.Refreshbtn addTarget:self action:@selector(Refreshbtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [manger connectWithUrlStr:urlString completion:^(id respondObject) {
        if ([respondObject isKindOfClass:[NSError class]]) {
            ...
            if ([[dic objectForKey:@"code"] isEqualToString:@"0"]) {
                ...
                [MBProgressHUD hideAllHUDsForView:self animated:YES];
                [self.JYtableview reloadData];
                [self.JYtableview headerEndRefreshing];
                refreshview.hidden=YES;
...
-(void)initViews
{
    self.JYtableview=[[UITableView alloc]initWithFrame:CGRectMake(0, 109, KDeviceWidth, KDeviceHeight-110) style:UITableViewStyleGrouped];
...
- (IBAction)fenleixuanzebtn:(id)sender {
    ...
    if (NOK==NO) {
        ...
        self.image.image=ima;
        self.subtableview =[[UITableView alloc]initWithFrame:CGRectMake(0, 105, 90, 90)];
